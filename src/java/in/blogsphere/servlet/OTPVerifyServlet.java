...
public class OTPVerifyServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String userOTP = request.getParameter("user-otp");
        String button = request.getParameter("button");

        HttpSession session = request.getSession();

        if (userOTP == null || !userOTP.equals(String.valueOf(session.getAttribute("otp")))) {
            request.setAttribute("otpstatus", "Wrong OTP, please try again");
            ...
            if (userid != null) {
                response.addCookie(CookieUtil.build("blogsphere-user", userid, 2500000));

                if (button.equals("f")) {
                    response.sendRedirect("index.jsp");
                } else {
                    response.sendRedirect("profile-details.jsp");
                }
            } else {
